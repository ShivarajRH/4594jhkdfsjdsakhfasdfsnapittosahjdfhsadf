...
//		$this->db->query("select 1 from king_admin where id=? and password=?",array($user['userid'],md5($exp)))->
	...
	function do_shipment_batch_process()
	{
		...
		$process_partial_byorder_qty = $this->input->post('process_partial_byorder_qty');
		$user = $this->erpm->auth();
		...
		}else
		{
			...
			if($by_menu)
			{
				...
				if($pmenu_idlist)
				{
					$ps_part_byordqty = $process_partial_byorder_qty?1:0;
		...
		if($i_transid)
			$raw_trans=$this->db->query("select o.*,t.partner_reference_no from king_transactions t join king_orders o on o.transid=t.transid and o.status=0 where t.batch_enabled=1 $cond and t.transid=?",$i_transid)->result_array();
		else
		...
		$partial_qty_alloted_order = array();
		foreach($trans as $transid=>$orders)
		{
			...
			foreach($orders as $order)
			{
				...
				foreach($products[$itemid] as $p)
				{
					...
					if($process_stk_chk)
					{
						...
						$rp_qty = $p['qty']*$order['quantity'];
						//echo ($stock[$p['product_id']]).'-'.$p['product_id'].' - '.$p['qty'].' - '.$order['quantity'].' ';
						if($stock[$p['product_id']] < $rp_qty)
						{
							if($ps_part_byordqty == 0)
							{
								$pflag=false;
								break;
							}
				...
				foreach($orders as $order)
				{
					if(in_array($order['id'],$same_order))
					{
						...
						foreach($products[$order['itemid']] as $p)
						{
							...
							if($ps_part_byordqty)
							{
								if($stock[$p['product_id']])
								{
									if($stock[$p['product_id']] < $p['qty']*$order['quantity'])
									{
										$partial_qty_alloted_order[$order['id']] = $stock[$p['product_id']];
										$stock[$p['product_id']] = 0;
									}else
									{
								...
								}else
								{
									...
									// remove selected order from the process orders array 
									$k = array_search($order['id'],$to_process_orders);
									if(is_int($k))
									{
										$to_process_orders = array_unique($to_process_orders);
										unset($to_process_orders[$k]);
									}
								}
							}else 
							{
			...
			if(count($to_process_orders))
				$total_trans = $this->db->query("select count(distinct transid) as ttl from king_orders where id in (".implode(',',$to_process_orders).") ")->row()->ttl;
			else
				$total_trans = 0;
			
			
			
			if($total_trans>=$ttl_num)
				break;
		}

		if(empty($to_process_orders) && empty($partial_qty_alloted_order))
		{
		...
		if(count($partial_qty_alloted_order) && $ps_part_byordqty)
		{
				$orders=array_keys($partial_qty_alloted_order);
				
				$orders = array_merge($to_process_orders,$orders);
				
				foreach ($partial_qty_alloted_order as $pordid => $pord_newqt)
				{
					$n_oid=random_string("numeric",10);
					$p_order=$this->db->query("select * from king_orders where id=?",$pordid)->row_array();
					$prod=$this->db->query("select name from king_dealitems where id=?",$p_order['itemid'])->row_array();
					$n_qty=$p_order['quantity']-$pord_newqt;
					
					$this->db->query("insert into t_process_partialqty_orders (transid,oid,new_oid,qty,new_qty,created_on,created_by) values (?,?,?,?,?,now(),?) ",array($p_order['transid'],$pordid,$n_oid,$p_order['quantity'],$pord_newqt,$user['userid']));
					
					$inp_o = $p_order;
					unset($inp_o['sno']);
					$inp_o['id'] = $n_oid;
					$inp_o['quantity'] = $n_qty;
					$inp_o['status'] = 0;
					$inp_o['actiontime'] = time();
					$this->db->insert("king_orders",$inp_o);
					
					$this->db->query("update king_orders set quantity=? where id=? limit 1",array($pord_newqt,$pordid));
					$this->erpm->do_trans_changelog($p_order['transid'],"Order '{$prod['name']}' quantity changed from {$p_order['quantity']} to {$pord_newqt}");
					
				}
		}else
		{
			$orders = $to_process_orders;
		}
		...
		if($process_partial)
			$process_type = 2;
		else if($process_partial_byorder_qty)
			$process_type = 3;
		...
		if(!empty($invoices))
		{
			for($b=0;$b<$ttl_batchs;$b++)
			{
				...
				$this->db->query("insert into shipment_batch_process(num_orders,process_type,batch_remarks,created_on) values(?,?,?,?)",array($ttl_inbatch,$process_type,$batch_remarks,date('Y-m-d H:i:s')));
		...
		foreach($orders as $o)
		{
			$pinv_det=$this->db->query("select id,p_invoice_no from proforma_invoices where order_id=? order by id desc ",$o)->row_array();
			$invid = $pinv_det['id'];
			$pinvno = $pinv_det['p_invoice_no'];
			
			$s_prods=$this->db->query("select t.partner_reference_no,o.transid,o.id,p.product_id,p.qty,o.quantity,o.i_orgprice,o.id as order_id from king_orders o join m_product_deal_link p on p.itemid=o.itemid join king_transactions t on t.transid  = o.transid  where o.id=? order by o.sno asc",$o)->result_array();
			...
			$s_prods_1=$this->db->query("select t.partner_reference_no,o.transid,o.id,go.product_id,p.qty,o.quantity,o.i_orgprice,o.id as order_id 
												...
												where o.id=? order by o.sno asc",$o)->result_array();
			...
			foreach($s_prods as $p)
			{
				...
				*/
				...
				$stk_prod_list = $this->db->query($sql,$p['product_id'])->result_array();
				
				// iterate all stock product 
				foreach($stk_prod_list as $stk_prod)
				{
					$reserv_qty = 0; 
...
*/
	...
	function do_stock_log($in,$qty,$pid,$grn_iid,$corp=false,$cancel_invoice=false,$proforma_invoice=false,$mrp_change_updated=-1,$return_product_id=0)
	{
		if($cancel_invoice)
			$in=0;
		...
		$this->db->query("insert into t_stock_update_log(product_id,update_type,p_invoice_id,corp_invoice_id,invoice_id,grn_id,qty,current_stock,created_on,created_by,mrp_change_updated,return_prod_id) values(?,?,?,?,?,?,?,?,now(),?,?,?)",$inp);
	}
	...
	function do_pack()
	{
		...
		foreach($orders as $o)
		{
			...
			foreach($pls as $p)
			{
				foreach($imeis[$p['product_id']] as $il=>$imei)
				{
					if($imei==0)
						continue;
					$this->db->query("update t_imei_no set order_id=?,is_returned=0,status=1 where imei_no=? limit 1",array($o['id'],$imei));
				}
			}
		}
		$invoice_no=$inv[0];
		...
		$user = $this->erpm->auth();
		$orders_det=$this->db->query("select a.quantity as qty,a.itemid,a.id,b.franchise_id,b.is_pnh from king_orders a join king_transactions b on b.transid=a.transid where a.id in ('".implode("','",$p_oids)."')")->result_array();
				...
				foreach($imeis[$p['product_id']] as $il=>$imei)
				{
					...
					if($o['is_pnh'])
					{
						...
						if($is_voucher_book_deal)
						{
							...
							//get book vouchers
							$book_vouchers_res=$this->db->query("select * from  pnh_t_book_voucher_link where book_id=?",$book_details['book_id']);
							if($book_vouchers_res->num_rows())
							{
								foreach($book_vouchers_res->result_array() as $voucher)
								{
									$this->db->query("update pnh_t_voucher_details set franchise_id=?,is_alloted=1,alloted_on=?",array($o['franchise_id'],cur_datetime()));
								}
							}
						}
					}
		...
		{
			list($itemid,$pid,$pbcode) = explode('_',$stk_prod_sel);
		...
		if($batch_id)
		{
			...
			$refund_res = $this->db->query('select a.p_invoice_no,c.quantity,di.is_combo,di.is_pnh,c.itemid,d.menuid,c.transid,a.order_id,a.product_id,i_orgprice as ordmrp,i_orgprice-(i_price-i_coup_discount) as disc,b.mrp,(a.qty-ifnull(a.release_qty,0)+ifnull(a.extra_qty,0)) as allot_qty 
													...
													where batch_id = ? and a.p_invoice_no = ? and a.status = 1  
													order by a.order_id 
			...
			if($inv_trans_det['is_pnh'])
			{
				...
				$invoice_credit_note_amt = $this->db->query("select sum(credit_note_amt*quantity) as amt from king_invoice a join king_orders b on b.id = a.order_id where a.invoice_no = ? 
 ",$invoice_no)->row()->amt;
				if($invoice_credit_note_amt)
				{
					$arr = array($inv_trans_det['franchise_id'],7,$inv_trans_det['invoice_no'],$invoice_credit_note_amt,'credit_note',1,date('Y-m-d H:i:s'),$user['userid']);
					...
					$arr = array($inv_trans_det['franchise_id'],$inv_trans_det['invoice_no'],$invoice_credit_note_amt,date('Y-m-d H:i:s'),$user['userid']);
					$this->db->query("insert into t_invoice_credit_notes (franchise_id,invoice_no,amount,created_on,created_by) values(?,?,?,?,?)",$arr);
		...
		/*
		// stock update by selected barcodes 
		foreach($pbcodes as $stk_prod_sel => $stk_mrp_det)
		{
			list($itemid,$pid,$pbcode) = explode('_',$stk_prod_sel);
			list($stk_cnt,$pmrp,$stk_id) = explode('_',$stk_mrp_det);
			...
			// get resered qty for product from stock and revert back to stock master
			$p_reserv_qty = 0;
			$reserv_stk_res = $this->db->query('select id,stock_info_id,qty from t_reserved_batch_stock where batch_id = ? and order_id = ? and product_id = ? and released_on is null ',array($batch_id,$itemid,$pid));
			if($reserv_stk_res->num_rows())
			{
				foreach($reserv_stk_res->result_array() as $reserv_stk_det)
				{
					if(($stk_id == $reserv_stk_det['stock_info_id']))
					{
						if(($stk_cnt < $reserv_stk_det['qty']))
						{
							$release_qty = $reserv_stk_det['qty']-$stk_cnt;		
						}else
						{
							$release_qty = $stk_cnt-$reserv_stk_det['qty'];
						}
					}
						
					
					// some qty need to be reverted back 
					if($release_qty)
					{
						$sql="update t_stock_info set available_qty=available_qty+? where product_id=? and batch_id = ? and item_id = ? and stock_id = ? limit 1";
						$this->db->query($sql,array($release_qty,$pid,$batch_id,$itemid,$reserv_stk_det['stock_info_id']));
					}
					$this->db->query("update t_reserved_batch_stock set release_qty=? and released_on = ? where id = ? ",array($release_qty,time(),$reserv_stk_det['id']));
				}
			}
			*/
			...
			$proforma_inv_id = $this->db->query("select a.id as id from proforma_invoices a 
														join king_orders b on a.order_id = b.id 
														where b.itemid = ? and a.p_invoice_no = ?  ",array($itemid,$p_invoice))->row()->id;
			$this->erpm->do_stock_log(0,$stk_cnt,$pid,$proforma_inv_id,false,true,true);
		...
		*/
		...
		if($inv_trans_det['is_pnh'])
		{
			$this->db->query("update shipment_batch_process_invoice_link set invoice_no=$invoice_no,invoiced_on=now(),invoiced_by={$user['userid']} where p_invoice_no=?",$p_invoice);
	...
	function do_outscan($awb,$partner_id="")
	{
		...
		if($partner_id == 5 )
		{
			...
			$bpinv = $this->db->query("select d.*,c.partner_reference_no,e.is_manifesto_created,e.courier_name 
							from king_invoice a 
							join king_orders b on a.order_id = b.id 
							...
							limit 1;",array($partner_id,$awb,$awb))->row_array();
		...
		if(empty($bpinv))
			die("<div class='not_found' style='background:#cd0000;'>AWB/Invoice/Order No:{$awb} not found</div>");
		...
		if($this->db->query("select invoice_status as s from king_invoice where invoice_no=?",$bpinv['invoice_no'])->row()->s=="0")
			die("<div class='cancelled' style='background:green;'>Invoice/Order No:{$awb} is cancelled</div>");
		
		if($bpinv['shipped'])
			die("<div class='already_shipped' style='background:orange;'>Invoice/Order No:{$awb} already shipped</div>");
		
		if($bpinv['packed'] && $is_pnh)
			die("<div class='already_packed' style='background:#cd0000;'>Invoice/Order No:{$awb} already Packed</div>");
		...
		if($bpinv && $partner_id)
		{
			if($partner_id==5)
			{
				if(!$bpinv['is_manifesto_created'])
					die("<div class='no_manifesto' style='background:#cd0000;'>Order No:{$bpinv['partner_reference_no']} is not in manifesto</div>");					
					
				if(!in_array($bpinv['courier_name'],$courier_names))
					die("<div class='courier_mismatch' style='background:purple;'>Order No:{$bpinv['partner_reference_no']} is not in <b>".implode(',',$courier_names)."</b> </div>");					
		...
		{
			die("<div class='courier_mismatch' style='background:purple;'>Delivery Hub not found - ".$terr_det['town_name']."</div>");
		}
		...
		//<!--pnh tray module end-->
			...
			if($tray_terr_det)
			{
				...
				die("<div class='outscanned' style='background:#fcfcfc;'>Invoice No {$awb} ,".$terr_det['territory_name']." </br> Packed - TRAY No:".($tray_terr_det['tray_name'])." <script>$('#working_tray b').html('{$tray_terr_det['tray_name']}');</script></div>");
			}else{
				die("<div class='outscanned' style='background:#fcfcfc;'>All the trays are in use please add a new tray</div>");
			}
		}else
		{
			...
			die("<div class='outscanned' style='background:#fcfcfc;'>Invoice/Order No {$awb} outscanned</div>");
		}
...
*/		
	...
	function getinvoiceforpacking($inv_no)
	{
		...
		$ret=$this->db->query("select d.pic,d.is_pnh,e.menuid,i.discount,p.product_id,p.mrp,p.barcode,i.p_invoice_no,p.product_name,o.i_orgprice as order_mrp,o.quantity*pl.qty as qty,d.name as deal,d.dealid,o.itemid,o.id as order_id,i.p_invoice_no from proforma_invoices i join king_orders o on o.id=i.order_id join m_product_deal_link pl on pl.itemid=o.itemid join m_product_info p on p.product_id=pl.product_id join king_dealitems d on d.id=o.itemid join king_deals e on e.dealid=d.dealid where i.p_invoice_no=? and i.invoice_status=1",$inv_no)->result_array();
		$ret2=$this->db->query("select d.pic,d.is_pnh,e.menuid,i.discount,i.discount,p.product_id,p.mrp,p.barcode,i.p_invoice_no,p.product_name,o.i_orgprice as order_mrp,o.quantity*pl.qty as qty,d.name as deal,d.dealid,o.itemid,o.id as order_id,i.p_invoice_no from proforma_invoices i join king_orders o on o.id=i.order_id join products_group_orders pgo on pgo.order_id=o.id join m_product_group_deal_link pl on pl.itemid=o.itemid join m_product_info p on p.product_id=pgo.product_id join king_dealitems d on d.id=o.itemid join king_deals e on e.dealid=d.dealid where i.p_invoice_no=? and i.invoice_status=1",$inv_no)->result_array();
			...
			foreach($po as $p)
			{
				...
				if($p['rqty']!=0)
				{
					...
					else
					{
						...
						//echo $upd_stat.'_'.$ins_stat;
						
						
						/*
						$stk_prodchkdet_res_1 = $this->db->query("select 1 from t_stock_info where product_id=? and location_id=? and rack_bin_id=? and mrp=? and (product_barcode = ? or length(trim(product_barcode))=0) ",array($p['product'],$p['location'],$p['rackbin'],$p['mrp'],$p['barcode']));
						
						if($stk_prodchkdet_res_1->num_rows())
						{
							$this->db->query("update t_stock_info set product_barcode = ?,available_qty=available_qty+?,modified_on=now() where product_id=? and location_id=? and rack_bin_id=? and mrp=? and (product_barcode = ? or length(trim(product_barcode))=0)  limit 1",array($p['barcode'],$p['rqty'],$p['product'],$p['location'],$p['rackbin'],$p['mrp'],$p['barcode']));
						}
						else
					...
					$this->erpm->do_stock_log(1,$p['rqty'],$p['product'],$grn,false,false,false,$p['upd_pmrp']);
				}
			}
...
//		$this->db->insert_batch("pnh_m_device_info",$inp);
	...
	function do_pnh_offline_order()
	{
		...
		foreach($items as $i=>$item)
		{
			$prod=$this->db->query("select i.*,d.publish,c.loyality_pntvalue from king_dealitems i join king_deals d on d.dealid=i.dealid JOIN pnh_menu c ON c.id = d.menuid where i.is_pnh=1 and  i.pnh_id=? and i.pnh_id!=0",$item['pid'])->row_array();
			if(empty($prod))
		...
		foreach($items as $item)
		{
			...
			for($qi=0;$qi<$ttl_qty;)
			{
				...
				if($split_order)
				{
					...
					$this->db->query("insert into pnh_member_info(pnh_member_id,franchise_id,created_by,created_on)values(?,?,?,?)",array($membr_id,$fid,$admin['userid'],time()));
				}	
	...
	function sendsms_franchise_order($transid = '',$d_total=0)
	{
		...
		$sql_trans = "select a.id,a.itemid,b.name as itemname,i_orgprice,login_mobile1,i_price,i_coup_discount,i_discount,a.quantity,c.menuid,a.transid,f.franchise_id,f.franchise_name   
							from king_orders a 
		...
		if($res_trans->num_rows())
		{
			foreach($res_trans->result_array() as $row_trans)
			{
				...
				if(!in_array($row_trans['menuid'],$pnh_beauty_menu))
				{
					...
					$order_product_list[] = $row_trans['itemname'].'('.$row_trans['quantity'].' Qty,'.$item_land_price.')';
				}
			}
		}
...
select statement_id,1 as type,(a.invoice_no) as invoice_no,sum(credit_amt) as credit_amt,sum(debit_amt) as debit_amt,date(a.created_on) as action_date,remarks
		...
		where action_type = 7 and type is null  
		and a.franchise_id = ?  
		and unix_timestamp(a.created_on) between unix_timestamp(?) 
		and unix_timestamp(?)  
...
order by action_date 
  ...
  /**
   ...
   function do_pnh_update_execu_account_log()
   {
	...
	function get_pnh_invreturns($pg)
	{
	...
	function get_pnh_invreturns_ttl()
	{
		$sql = "select count(*) as ttl from pnh_invoice_returns ";
		...
		if($res->num_rows())
		{
			...
			}else 
			{
				...
				foreach ($invord_list as  $ord)
				{
					...
					{
						$ttl_pending_inreturn_qty = $this->db->query("select ifnull(sum(qty),0) as t from pnh_invoice_returns_product_link where order_id = ? and product_id = ? ",array($ord['order_id'],$prod_det['product_id']))->row()->t;
	...
	function get_fran_availcreditlimit($fr_id=0)
	{
		...
		$pending_payment = ($fr_accdet['shipped_tilldate']-$fr_accdet['paid_tilldate']+$fr_accdet['acc_adjustments_val']);
	 ...
	 */
	function is_prepaid_franchise($fid='')
	{
		...
		if($franchise_menu_det)
		{
			...
			//check if it is prepaid franchise then no credit
