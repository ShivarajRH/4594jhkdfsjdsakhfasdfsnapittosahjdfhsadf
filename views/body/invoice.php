...
$invoice_credit_note_res = $this->db->query("select id,amount from t_invoice_credit_notes where invoice_no = ? order by id desc limit 1 ",$invoice_no);
...
$is_voucher_order_res=$this->db->query("select voucher_payement from king_transactions where transid=? and is_pnh=1",$orders[0]['transid']);
...
if($is_voucher_order_res->num_rows())
{
	...
	if($is_voucher_order['voucher_payement'])
	{
		$show_voucher_det=1;
...
<?php }?>	
		<div id="inv_logo_area">
			...
			<div  id="top_tax_inv_bar" style="border-bottom:2px solid #555;padding:15px;min-height: 40px;">
		...
		<div id="pnh_inv_print_header" style="margin-top:40px;;border-bottom:2px solid #000;padding:5px;font-weight:bold;text-align:center;overflow: hidden;display: none;text-align: center">
			<br>
			<br>
...
foreach($orders as $order){
	...
	// displaying returned product list
	/*  	
...
?>			
			...
			<tr>
				<td>
					<?php echo ($order['status'] == 4)?'<strike>':'';?>
					<?=$order['name'].($order['pnh_id']?' - <b>'.$order['pnh_id'].'</b>':'')?>
					
					<?php echo ($order['status'] == 4)?'</strike> - Product Returned ':'';?>
...
<?php if($is_pnh) { ?>
		...
		$('.pnh_logo').hide();
	var inv_html = '<div style="margin-top:20px;">'+$("#invoice").html().replace('TAX INVOICE','TAX INVOICE - Customer Copy');
		$('.pnh_logo').show();
