									...
									$.each(prod1,function(j,prod){
										if((item.quantity*prod.qty - prod.pen_return_qty) != 0)
										{
											...
											if(j==0)
											{
												if(i==0)
												{
													rowHtml += '	<td rowspan="'+rspan+'">'+item.invoice_no+' <br > <span style="font-size:11px;"><a href="'+site_url+'/admin/pnh_franchise/'+resp.franchise_id+'">'+resp.franchise_name+'</a></span> </td>';
												}
												cspan = 1;
												rspan = 0;
												if(rspan)
