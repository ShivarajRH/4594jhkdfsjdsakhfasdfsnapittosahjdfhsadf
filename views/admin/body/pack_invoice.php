...
<table class="datagrid" style="margin-top: 20px;" width=100%>
	...
	<tbody>
		...
		<?php if(!empty($imeis)){
				...
				foreach($imeis as $im)
					$prod_imei_list[$im['imei_no']] = $i['product_id'];
				echo '<ol class="imei_inp_list">';
				for($p=0;$p<$i['qty'];$p++)
				{
		?>
					<li><input type="text" readonly="readonly" class="imei<?=$i['product_id']?> imei<?=$i['product_id']?>_unscanned imeis imeip<?=$p?>" style="width: 100px;padding:2px;font-size: 9px" value="" ></li> 
		<?php 
				}
				echo '</ol>';
		}?>
				...
				<tr>
					...
					<div class="scan_proditems" style="width: 30px; text-align: center;">
						...
						?>
							...
							pid="<?php echo $prd_id;?>"
							name="pbc[<?php echo $i['itemid'].'_'.$prd_id.'_'.($mrp_b[0]?$mrp_b[0]:'BLANK').'_'.$mrp_b['stock_id'];?>]" value="0"
...
<script type="text/javascript">
	...
	<?php
		if($prod_imei_list)
			foreach($prod_imei_list as $p_imeino => $i_prod_id)
			{
	?>
				prod_imeino_list[<?php echo $p_imeino;?>] = <?php echo $i_prod_id;?>;
	<?php				
			} 
	?>
