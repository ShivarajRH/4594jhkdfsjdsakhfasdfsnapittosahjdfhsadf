		...
		<table class="datagrid" cellpadding="5" cellspacing="0" width="100%">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<?php if($det['status']==2){ echo 'Yes';}else{ echo 'No';}?>
					</td>
					<td><a href="javascript:void(0)" class="receipt_details" book_id="<?php echo $det['book_id']; ?>">View</a></td>
					<td><a href="javascript:void(0)" class="update_reciept_dlg" allotment_id="<?php echo $det['allotment_id'];?>">Update</a></td>
				</tr>		
				<?php	
					}
	...
	open:function(){
		...
		$.post(site_url+'/admin/jx_get_book_detby_allotment',{allotment_id:$(this).data('allotment_id')},function(res){
			...
			$.each(res.book_det,function(a,b){
				html_cnt+="<tr>";
				html_cnt+="		<td>"+(a+1)+"</td>";
				html_cnt+="		<td>"+b.book_type_name+"<input type='hidden' name='book_ids[]' value='"+b.book_id+"'><input type='hidden' name='franchise_ids[]' value='"+b.franchise_id+"'><input type='hidden' name='allotments_ids[]' value='"+b.allotment_id+"' ></td>";
				html_cnt+="		<td>"+b.book_value+"</td>";
				html_cnt+="		<td><input type='text' name='receipt_id[]' size='8'></td>";
				adjust_value='';
				adjust_value_pr='';
				if((b.book_value*1)==(b.payed*1))
				{
					complete_ttl+=1;
