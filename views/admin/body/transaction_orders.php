...
<form method="post" onsubmit='return confirm("Are you sure?")' action="<?=site_url("admin/endisable_for_batch/{$order['transid']}")?>">
	<input type="submit" value="<?=$tran['batch_enabled']?"Dis":"En"?>able for Batch">
</form>
</div>
<?php if($tran['batch_enabled']){?>
<div style="float:right;padding-right:20px;">
<form method="post" onsubmit='return confirm("Are you sure?")' action="<?=site_url("admin/add_batch_process")?>">
...
<h2>Order Transaction : <?=$tran['transid']?></h2>
<div class="clear"></div>
...
<table style="background:#FFFFEF;width:200px;" class="datagrid smallheader noprint">
...
<div style="margin:5px 0px;padding:5px;border:1px solid #aaa;">
...
<div <?=!$tran['priority']?"class='changeprior'":" title='{$tran['priority_note']}'"?> style="margin-top:10px;width:100px;display:inline-block;text-align:center;font-weight:bold;padding:15px 30px;border:1px solid #aaa;<?=$tran['priority']?"":"cursor:pointer;"?>background:<?=$tran['priority']?"yellow":"#ddd"?>;text-transform:uppercase;">
<?php if($tran['priority']){?>HIGH<?php }else{?>NORMAL<?php }?>
<br>priority
</div>

<div style="margin-top:10px;width:100px;display:inline-block;text-align:center;font-weight:bold;padding:15px 30px;text-transform:uppercase;background:#fafafa;border:1px solid #ccc;">
...
<table class="datagrid smallheader noprint" style="min-width:250px;">
...
<a target="_blank" href="<?=site_url("admin/pnh_cash_bill/{$tran['transid']}")?>">Print Cash Bill</a>
</div>
<?php }?>
...
<input type="button" value="Reship items of this order" id="reship_button">
</div>
...
<div id="orders_data">
	...
	<td>
		...
		<div>
			...
			<div style="display:none;padding:5px;background:#fff;border:1px dashed #444;">
				...
				<input type="button" class="changeqtyorder" value="Update">
			</div>
		</div>
		<?php }?>
	</td>
	...
	<div>Rs<?php if(!$is_prepaid){?> <?=(($o['i_price']-$o['i_coup_discount'])*$o['quantity']);}else{?> <?=(($o['i_orgprice']-$o['i_coup_discount'])*$o['quantity']);}?></div>
	</td>
	<td>
	...
	<?php if($cancelsingle){?><input type="submit" value="Cancel selected orders"><?php }?> <?php if($cancelall){?><input type="button" value="Cancel all orders" id="cancel_all"><?php }?>
	</div>
	<?php }?>
	</form>
	...
	<div style="width:40%;padding-right:40px;float:left;">
	<h4>Changelog &amp; messages &nbsp; &nbsp; <a onclick='showaddmsg()' style="font-weight:normal" href="javascript:void(0)">Add a msg</a></h4>
	<div id="add_msg_cont">
	<form method="post">
	...
	<h4>Support Tickets for this transaction &nbsp; &nbsp; <a target="_blank" style="font-weight:normal" href="<?=site_url("admin/addticket/{$tran['transid']}")?>">Raise a ticket</a></h4>
	<table class="datagrid smallheader" width="100%">
...
<table class="datagrid">
<thead><Tr><th>Sno</th><th>Product Name</th><th>MRP</th><th>Offer price/ Dealer price</th><th>Menu Margin (A)</th><th>Scheme discount (B)</th><th>Balance Discount (C)</th><th>Voucher Margin(D)</th><th>Total Discount (A+B+C+D)</th><th>Unit Price</th><th>Qty</th><th>Order price</th><th>Redeem value</th></Tr></thead>
<tbody>
<?php $i=1; foreach($this->db->query("SELECT p.*,i.name,o.i_orgprice AS mrp,o.i_price AS price,c.loyality_pntvalue,o.redeem_value
										...
										JOIN pnh_menu c ON c.id = b.menuid  where p.transid=?",$tran['transid'])->result_array() as $item){?>
<tr>
<td><?=$i++?></td>
<td><?=$item['name']?></td>
<td><?=$item['mrp']?></td>
