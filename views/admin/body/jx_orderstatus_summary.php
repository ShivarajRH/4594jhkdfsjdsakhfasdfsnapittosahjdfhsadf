...
//    echo $cond; die();
        ...
        $sql_shipped = "select distinct b.transid,sum(o.i_coup_discount) as com,c.amount,o.transid,o.status,o.time,o.actiontime,pu.user_id as userid,pu.pnh_member_id 
                            ...
                            where o.status = 2 and sd.shipped = 1 and is_pnh = 1 $cond and sd.shipped_on between from_unixtime($st_ts) and from_unixtime($en_ts) 
                            group by b.transid 
        ...
        $sql_removed="select tr.transid,sum(o.i_coup_discount) as com,tr.amount,o.transid,o.status,o.time,o.actiontime,mi.user_id as userid,mi.pnh_member_id from king_orders o
...
//                    PAGINATION
                    ...
                    $config['base_url'] = site_url("admin/jx_orders_status_summary/".$type.'/'.$date_from.'/'.$date_to.'/'.$terrid.'/'.$townid.'/'.$franchiseid); //site_url("admin/orders/$status/$s/$e/$orders_by/$limit");
