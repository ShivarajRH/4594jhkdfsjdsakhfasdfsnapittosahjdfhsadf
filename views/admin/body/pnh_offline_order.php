...
<a href="javascript:void(0)" onclick="load_scheme_details()" style="float: right;margin-top: -39px;margin-right:273px;" class="button">Scheme & Menu Details</a>
<a href="<?php echo site_url('/admin/pnh_deals')?>" target="_blank" style="float:left;margin-top: -38px;margin-left:1099px;" class="button">Go To Deals</a>
...
<div id="prod_suggest_list" style="display:none">
...
<div style="margin:5px 0px;background:#fafafa;border:1px solid #aaa;padding:5px;width:68% !important;">
<div style="clear: both">
		...
		<h4 style="margin:0px;">Order for Member</h4>
		
		Enter MEMBER ID (MID) : <input style="font-size:120%" maxlength="8" type="text" class="inp mid inp_focus" name="mid" size=18>
		<div style="clear:both;overflow: hidden;background: #fcfcfc">
			<div id="member_ids" style="float: right;clear: right;padding: 4px 6px;background: #FFFFA0;font-size: 11px;text-align: center;">
...
<thead><tr><th>Sno</th><th>Product Image-PID</th><th>Product Name</th><th>MRP</th><th>Offer price / DP price</th><th>Margin(%)</th><th>Landing Cost</th><th>Quote</th><th>Qty</th><th>Sub Total</th><th></th></tr></thead>
<tbody>
</tbody>
</table>
...
<div id="price_changes" style="float:right;margin-right:30px;width:700px;">
</div>
...
<td>%sno%</td>
<td><img alt="" height="100" src="<?=IMAGES_URL?>items/%pimage%.jpg" style="float:right;margin-right:20px;"><span>%pid%</span></td>
...
text-align: center;"><b>OldMRP:</b> <span style="color: #cd0000;font-size: 13px;">%oldmrp%</span></div>
</td>
<td><span class="price">%price%</span></td>
<td>%margin%%</td>
<td><span style="background-color:#89c403;display:block;padding:12px 15px;"><b>%lcost%</b></span></td>
<td><input type="text" name="quote[]" size=4></td>
<!--<td>%src%</td>-->
<td><input type="text" class="qty" size=2 name="qty[]" value="1"></td>
<td><span class="stotal">%price%</span></td>
...
function select_fran(fid)
{
	...
	$.post("<?=site_url("admin/pnh_jx_getfranbalance")?>",{id:fid},function(data){
		...
		$("#fran_name_disp").html(" for '<a href='<?=site_url("admin/pnh_franchise")?>/"+$("#i_fid").val()+"' target='_blank'>"+$("#franchise_det h4 a").text()+' '+$("#franchise_det h4 a").attr('loc')+"</a>' - Mobile: "+$(".ff_mob").text()+' '+fran_type_msg);
...
$(function(){
	...
	$("#order_form").submit(function(){
		...
		if($(".mid",$(this)).val().length==0)
		{alert("Enter MID");return false;}
		if(pids.length==0)
		...
		if(confirm("Total order value : Rs "+total+"\nAre you sure want to place the order?"))
		{
			...
			$.post("<?=site_url("admin/pnh_jx_checkstock_order")?>",{attr:attr,pids:ppids.join(","),qty:qty.join(","),fid:$('#i_fid').val(),mid:$(".mid").val()},function(data){
				obj=$.parseJSON(data);
				if(obj.e==0)
				{
					goodtogo=1;
	...
	$("#prods_order .qty").live("change",function(){
		...
		$(".stotal",p).html(parseFloat($(".lcost",p).html())*parseInt($(".qty",p).val()));
	});
	...
	$(".add_b_product").click(function(){
		...
		$.post("<?=site_url("admin/pnh_jx_loadpnhprodbybarcode")?>",{barcode:barcode},function(data){
...
.ui-dialog .ui-dialog-titlebar{background: #343434;}
