...
//			show_error("Invoice is not available for printing until its packed");
	...
	function print_allinv($is_active=1,$from="",$to="",$limit='',$st=''){
...
//		$is_active = $is_active?1:0; 
		...
		$inv_type = 'auditing';
		$sql = "select distinct invoice_no from king_invoice where date(from_unixtime(createdon)) >= ? and date(from_unixtime(createdon)) <= ?  and invoice_status = ? order by invoice_no ";
		...
		foreach($res->result_array() as $i=>$row){
			...
			if($q->num_rows()){
				...
				$data['trans']=$this->db->query("select * from king_transactions where transid=?",$orders[0]['transid'])->row_array();
				$data['inv_type'] = $inv_type;
				$this->load->view("body/invoice",$data);
			}
		}
