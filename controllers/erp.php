	...
	function cancel_invoice($invno="")
	{
		...
		if($inv_trans_det['is_pnh'])
		{
			...
			if($credit_note_id)
			{
				...
				$arr = array($inv_trans_det['franchise_id'],7,$inv_trans_det['invoice_no'],$inv_trans_det['inv_amt'],'credit note cancelled',1,date('Y-m-d H:i:s'),$user['userid']);
...
// 		$this->form_validation->set_rules('pissno', 'Is Active', 'required');
	...
	function pnh_jx_getfranbalance()
	{
		$user=$this->auth();
		$fid = $_POST['id'];
		$fran=$this->db->query("select datediff(curdate(),date(from_unixtime(created_on))) as reg_days,current_balance as balance,credit_limit as credit from pnh_m_franchise_info where franchise_id=?",$fid)->row_array();
	...
	function pnh_process_franchise_imei_activation()
	{
		...
		$this->form_validation->set_rules('srch_no','Member ID or Mobileno','required');
		$this->form_validation->set_rules('mem_type','Member Type','required');
		$this->form_validation->set_rules('franid','Franchise','trim|required');
		
		$mem_type = $this->input->post('mem_type');
		
		if(!$mem_type)
		{
			$this->form_validation->set_rules('mem_id','MemberID','trim|required|callback__validate_memberidfor_registration');
			$this->form_validation->set_rules('name','Name','trim|required');
		}
				
		$this->form_validation->set_rules('imeino_1','IMEINO 1','required|callback__validate_imeinoforactivation');
		$this->form_validation->set_rules('imeino_2','IMEINO 2','callback__validate_imeinoforactivation');
		...
		if($this->form_validation->run() == FALSE)
		{
			$this->pnh_franchise_activate_imei();
		}
		else
		{
			...
			$imeino_list[0] = $this->input->post('imeino_1');
			$imeino_list[1] = $this->input->post('imeino_2');
			$memid = $this->input->post('mem_id');
			$name = $this->input->post('name');
			$mobno = $this->input->post('mobno');
			
			// If membership type is new registration 
			if($mem_type == 0)
			{
				// Register New Member 
				if($this->db->query("select 1 from pnh_member_info where pnh_member_id=?",$mid)->num_rows()==0)
				{ 
					if($this->db->query("select 1 from pnh_m_allotted_mid where ? between mid_start and mid_end and franchise_id=?",array($memid,$franid))->num_rows()==0)
					...
					$this->db->insert("pnh_member_info",array("user_id"=>$userid,"pnh_member_id"=>$memid,"franchise_id"=>$franid,"first_name"=>$this->input->post("name"),"mobile"=>$this->input->post("mobno")));
					...
					$this->erpm->pnh_fran_account_stat($franid,1,PNH_MEMBER_FEE-PNH_MEMBER_BONUS,"50 Credit Points purchase for Member $mid","member",$memid);
				}
			}
			...
			$imeino_list = array_filter($imeino_list);
			
			$ttl_imei_actv_credit = 0;
			//Update IMEI/Serialno Activation Details to IMEI Table
			foreach($imeino_list as $imeino)
			{
				...
				if($this->db->affected_rows())
				{
					...
					$this->db->query("update king_orders set member_scheme_processed=1 where id=? and member_scheme_processed=0",$oid);
			...
			$sms_msg = 'Congratulations!!! Your IMEINO : %s'.implode(',',$imeino_list).' Activated';
	...
	function pnh_franchise($fid)
	{
		...
		$data['is_prepaid']=$this->erpm->is_prepaid_franchise($fid);
	...
	function pnh_bulk_sch_discount()
	{
		...
		if($_POST)
		{
			foreach(array("discount","start","end","reason","menu","brand","cat","fids","bulk_schtype","target_value","credit_prc","credit_value","mscheme_type","msch_applyfrm") as $i)
				$$i=$this->input->post($i);
			if(empty($fids))
			...
			foreach($fids as $fid)
			{
				...
				if($bulk_schtype==3)
				{
					$msch_applyfrm=strtotime($msch_applyfrm." 23:59:59");
					
					$inp=array("scheme_type"=>$mscheme_type,"franchise_id"=>$fid,"menuid"=>$menu,"categoryid"=>$cat,"brandid"=>$brand,"credit_value"=>$credit_value,"scheme_from"=>$start,"scheme_to"=>$end,"sch_apply_from"=>$msch_applyfrm,"created_by"=>$user['userid'],"created_on"=>time(),"is_active"=>1);
	...
	function pnh_suspend_fran($franchise_id)
	{
		...
		$this->db->query("update pnh_m_franchise_info set is_suspended=?,suspended_on=".time().",suspended_by={$user['userid']},reason=? where franchise_id=? limit 1",array($sus_type,$reason,$franchise_id));
		if($this->db->affected_rows()>0)
		{
...
//----------------------------------------------------END OF TASK PRINT----------------------------------------------------------------------     
	...
	/**
	 * function to build sub menu list by url 
	 */
	...
	function cm_generate_menu($ele,$menu_list,$wrp_start,$wrp_end,$selected_ids=array()){
		...
		if($menu_list){
			...
			foreach($menu_list as $menu){
				...
				$submenu_list = $ele->erpm->get_menubyparentid($menu['id']);
	
				$is_selected = ((in_array($menu['id'],$selected_ids))?'checked':'');
	
				echo '<li>';
					echo ' <a href="'.$link.'">'.$menu['name'].'</a>';
				if($submenu_list)
					$this->cm_generate_menu($ele,$submenu_list,'<ul class="left-sub-menu">','</ul>',$selected_ids);
				echo '</li>';
			}
			echo $wrp_end;
		}
	...
	function fmenu()
	{
		$menus_det=$this->db->query("select * from m_erp_menu where 1 order by parent,menu_order asc")->result_array();
		$usr_menus_det_res=$this->db->query("select * from m_erp_menu 
					where menu_role_const in (select const_name from user_access_roles where (value&341) != 0 ) 
				order by parent,menu_order asc ")->result_array();
		$allowed_menu_ids = array();
		foreach($usr_menus_det_res as $usrmenudet)
		{
			array_push($allowed_menu_ids,$usrmenudet['id']);
		}
		build_menu($menus_det,'menu','submenu',$allowed_menu_ids);
	}
	 ...
	 *
	 */
	function pnh_invoice_returns($pg=0)
	{
		$this->erpm->auth(PNH_INVOICE_RETURNS|PNH_ADD_INVOICE_RETURN);
		$data['pnh_inv_returns_ttl'] = $this->erpm->get_pnh_invreturns_ttl();
		$data['pnh_inv_returns'] = $this->erpm->get_pnh_invreturns($pg);
		
		$this->load->library('pagination');
		$config['base_url'] = site_url('/admin/pnh_invoice_returns');
			...
			if($rp_prod_det)
			{
				...
				if($has_imei_no)
				{
					...
					if($p_imei_res->num_rows())
					{
						...
						$this->db->query("update t_imei_no set status = 0,return_prod_id = ? and order_id = 0 where order_id = ? and product_id = ? and imei_no = ? ",array($rp_prod_det['rp_id'],$rp_prod_det['order_id'],$rp_prod_det['product_id'],$rp_prod_det['imei_no']));
				...
				$sql_stk = "select d.product_id,stock_info_id,product_barcode,if(d.product_barcode=?,1,0) as bc_match   
								from shipment_batch_process_invoice_link a 
								join proforma_invoices b on a.p_invoice_no = b.p_invoice_no 
								join t_reserved_batch_stock c on a.p_invoice_no = c.p_invoice_no 
								join t_stock_info d on d.stock_id = c.stock_info_id 
								where invoice_no = ? and d.product_id = ? and status = 1 
				...
				if($stock_det_res->num_rows())
				{
					...
					if($this->db->query("select count(*) as t from pnh_invoice_returns_product_link where id = ? and is_stocked = 0 ",$return_prod_id)->row()->t)
					{
						...
						$inv_trans_det = $this->db->query("select franchise_id,a.transid,a.invoice_no,(b.i_orgprice-b.i_discount+b.i_coup_discount) as inv_amt 
									from king_invoice a
									join king_orders b on a.order_id = b.id 
	...
	function product_order_summary()
	{
		...
		function view_manifesto_sent_log($srch_invoice=0,$from_date=0,$to_date=0,$status=0,$hub_id=0,$pg=0)
		{
			$user=$this->auth(PNH_SHIPMENT_MANAGER);
			...
			$mainfesto_sent_det=$this->erpm->get_manifesto_sent_log_det(0,$pg,$from_date,$to_date,$srch_invoice,$status,$mid,$m_ids_by_hub);
			...
			//pagination block
			$this->load->library('pagination');
			$config['base_url'] = site_url('admin/view_manifesto_sent_log/'.$srch_invoice.'/'.$from_date.'/'.$to_date.'/'.$status.'/'.$hub_id);
		...
		function jx_cancel_processedreceipts()
		{
			...
			//{
				...
				$grp_msg="Dear $franchise_name ,cheque $cheque_number,for amount Rs.$fran_amount,is bounced and penalty for this Rs.$amount ,arrange for payment immediately.";
				
				if($sms && $cancel_status==2)
				{
					$this->erpm->pnh_sendsms($login_mobile1,"Dear $franchise_name ,cheque $cheque_number,for amount $fran_amount,is bounced and penalty for this $amount ,arrange for payment immediately.",$fid,0,$sms_type);
				}
				if($tm_sms && $cancel_status==2)
				{
					foreach($tm_mobno as $mob_no)
					{
						$this->erpm->pnh_sendsms($mob_no,"Dear $franchise_name ,cheque $cheque_number,for amount $fran_amount,is bounced and penalty for this $amount ,arrange for payment immediately.",$fid,$empid,$sms_type);
	...
	//	$data['page']='CopyOfpnh_superscheme_log';
		$this->load->view("admin",$data);
	}
	...
	function get_fran_acc_stat($fid=0)
	{
		$acc = array();
		$acc['ordered'] = $this->db->query("select sum((i_orgprice-(i_coup_discount+i_discount))*quantity) as amt from king_orders a join king_transactions b on a.transid = b.transid where franchise_id = ? ",$fid)->row()->amt*1;
		$acc['cancelled'] = $this->db->query("select sum((i_orgprice-(i_coup_discount+i_discount))*quantity) as amt from king_orders a join king_transactions b on a.transid = b.transid where franchise_id = ? and a.status = 3  ",$fid)->row()->amt*1;
		
		$acc['shipped'] = $this->db->query("select sum((i_orgprice-(i_coup_discount+i_discount))*quantity)  as amt 
														from king_orders a 
														join king_transactions b on a.transid = b.transid 
														join king_invoice c on c.order_id = a.id and invoice_status = 1 
														join shipment_batch_process_invoice_link d on d.invoice_no = c.invoice_no 
														where franchise_id = ? and d.shipped = 1 
														and a.status in (1,2)",$fid)->row()->amt*1;
														
		$acc['invoiced'] = $this->db->query("select (sum(debit_amt)-sum(credit_amt)) as amt from pnh_franchise_account_summary where franchise_id = ? and action_type = 1 ;",$fid)->row()->amt*1;
																
		$acc['invoiced_not_shipped'] = $this->db->query("select sum((i_orgprice-(i_coup_discount+i_discount))*quantity) as amt  
														from king_orders a 
														join king_transactions b on a.transid = b.transid 
														join king_invoice c on c.order_id = a.id and invoice_status = 1 
														join shipment_batch_process_invoice_link d on d.invoice_no = c.invoice_no 
														where franchise_id = ? and d.shipped = 0 
														and a.status in (1,2)",$fid)->row()->amt*1;
														
		$acc['receipt_added'] = $this->db->query("select sum(credit_amt) as amt from pnh_franchise_account_summary where franchise_id = ? and action_type = 3 and receipt_type = 1  and credit_amt != 0;",$fid)->row()->amt*1;
		$acc['receipt_cancelled'] = $this->db->query("select sum(debit_amt) as amt from pnh_franchise_account_summary where franchise_id = ? and action_type = 3 and receipt_type = 1  and debit_amt != 0;",$fid)->row()->amt*1;

		$acc['correction'] = $this->db->query("select (sum(credit_amt)-sum(debit_amt)) as amt  from pnh_franchise_account_summary where franchise_id = ? and action_type = 5 ;",$fid)->row()->amt*1;

		$acc['ttl_paid'] = ($acc['receipt_added']-$acc['receipt_cancelled']);

		$acc['ttl_invoiced_shipped'] = ($acc['invoiced']-$acc['invoiced_not_shipped']);

		$acc['ttl_pending_payment'] = $acc['ttl_paid']-$acc['ttl_invoiced_shipped']+$acc['correction'];
	...
	function jx_getpnh_voucher_activitylog($type='fff',$franchise_id='',$pg=0)
	{
		...
		if($type=='activated_vouchers')
		{
			...
			$activity_res="SELECT * FROM pnh_t_voucher_details WHERE is_activated=1 and status in (3,4,5) $cond limit $pg,$limit";
			$v_log_details=$this->db->query($activity_res);
				
			$tbl_head=array('slno'=>'Sl no','v_slno'=>'Voucher Slno','mid'=>'Member ID','alloted_on'=>'Alloted to Franchise on','activated_on'=>'Activated On');
		...
		elseif($type=='partially_redeemed_vouchers')
		{
			...
			$ttl_rows=$this->db->query($activity_ttl)->num_rows();
			$activity_res="SELECT * FROM pnh_t_voucher_details WHERE is_activated=1 AND `status`=5 $cond";
			$v_log_details=$this->db->query($activity_res);
			$tbl_head=array('slno'=>'Sl no','v_slno'=>'Voucher Slno','mid'=>'Member ID','alloted_on'=>'Alloted to Franchise on','activated_on'=>'Activated On');
			if($v_log_details->num_rows())
			{
				foreach($v_log_details->result_array() as $i=>$v_log_det)
				{
					$tbl_data[]=array('slno'=>$i+1,'v_slno'=>$v_log_det['voucher_serial_no'],'mid'=>$v_log_det['member_id'],'alloted_on'=>$v_log_det['alloted_on'],'activated_on'=>$v_log_det['activated_on']);
				}
			}
		}
	 ...
	 function pnh_give_member_sch($fid)
	{
		...
		if($mbrsch_applyfrm)
			$mbrsch_applyfrm=strtotime($mbrsch_applyfrm);
		
		// check if scheme is already available.
		if($this->db->query("select count(*) as t from imei_m_scheme where franchise_id = ? and menuid=? and categoryid=? and brandid=? and is_active = 1 ",array($fid,$mbr_schmenu,$mbr_schcat,$mbr_schbrand))->row()->t!=0)
		{
			$this->erpm->flash_msg("IMEI/Serialno Scheme Already Available");
			redirect("admin/pnh_franchise/$fid#sch_hist");
			exit;
		}
	...
	function jx_load_all_shipped_mobimei($type='shipped_imeimobslno',$franchise_id='0',$fil_type='',$pg='0')
	{
		$this->erpm->auth();
		...
		$cond='';
		if($franchise_id)
			$cond='AND t.franchise_id='.$franchise_id;
		if(1)
		{
			...
			$log_data="SELECT o.imei_reimbursement_value_perunit as imei_activation_credit,i.is_imei_activated,imei_activated_on,imei_no,p.product_id,date_format(from_unixtime(o.time),'%d %M %Y') as orderd_on,l.product_name,o.quantity,o.imei_reimbursement_value_perunit,o.id,inv.invoice_no,t.paid,r.scheme_type,r.credit_value,b.pnh_member_id
							FROM king_orders o 
							JOIN king_transactions t ON t.transid=o.transid
							JOIN m_product_deal_link p ON p.itemid=o.itemid
							JOIN t_imei_no i ON i.order_id=o.id 
			...
			$log_details=$this->db->query($log_data);
			if($log_details->num_rows())
			{
				$tbl_head=array("slno"=>"Sl no","pname"=>"Product Name","imei_no"=>"IMEI Slno","selling_price"=>"Selling Price","is_activated"=>"Activated","activated_on"=>"Activated on","msch_credit"=>"Activation Credit");
				foreach($log_details->result_array() as $i=>$log_det)
				{
					$msch_type=array("Fixed Fee","Percentage");
					$tbl_data[]=array("slno"=>$i+1,"pname"=>$log_det['product_name'].'<br><b>Invoice no :</b>'.anchor('admin/invoice/'.$log_det['invoice_no'],$log_det['invoice_no']).'<br><b>Orderd on :'.$log_det['orderd_on'],"imei_no"=>$log_det['imei_no'],"selling_price"=>$log_det['paid'],"msch_credit"=>'Rs '.$log_det['imei_reimbursement_value_perunit'] .'<br><b>Scheme Type : </b>'.$msch_type[$log_det['scheme_type']].'<br><b>Credit value :</b> '.$log_det['credit_value'],"is_activated"=>($log_det['is_imei_activated']?'Yes':'No'),"activated_on"=>(format_datetime($log_det['imei_activated_on'])));
				}
			}
		}
		...
		{	
			...
			$tbl_data_html = '<table cellpadding="5" cellspacing="0" class="datagrid datagridsort" width="100%">';
			$tbl_data_html .= '<thead>';
			foreach($tbl_head as $th)
				$tbl_data_html .= '<th>'.$th.'</th>';
		...
		$config['base_url'] = site_url('admin/jx_load_all_shipped_mobimei/'.$type.'/'.$franchise_id.'/',$pg);
		...
		echo json_encode(array('log_data'=>$tbl_data_html,'pagi_links'=>$pagi_links,'type'=>'shipped_imeimobslno','franchise_id'=>$franchise_id,'pg'=>$pg));
	...
	function jx_searchbykwd()
	{
		...
		if(strlen($kwd) > 2)
		{
			...
			if($item_list_res->num_rows())
			{
				...
				{
					if($i < 5)
						echo '<a href="'.site_url('admin/viewproduct/'.$itm['product_id']).'"><span class="searchheading">'.$itm['product_name'].'</span></a>';
					$i++;
				}
				if($i==6)
					echo '<a href="javascript:void(0)"><span class="searchheading viewall">View All</span></a>';
