...
class Pnh extends Controller{
	...
	function process()
	{
		...
		{
			if($call == 'paid' || $call == 'ship' || $call == 'new' || $call == 'existing' || $call == 't'|| $call == 'd'|| $call == 'r' || $call == 'e'|| $call == 'm' || $call == 'bank' || $call=='lr')
			{
				if($call == 'bank')
		...
		if(count($invoice_nos))
		{
			...
			if(count($fran_invoice_delivered))
			{
				foreach($fran_invoice_delivered as $f_id=>$invoice_nos)
				{
					...
					$this->erpm->pnh_sendsms($fran_det['login_mobile1'],"Dear ".$fran_det['franchise_name'].",Invoice no-  ".implode(',',$invoice_nos)."is delivered.Happy Franchising!!!",$f_id,$emp_id);
				}
			} 
	...
	function returnd_invoice_frm_driver($from,$msg,$fran_ex)
	{
		...
		if(count($fran_invoice_returned))
		{
			...
			{
				$fran_det = $this->db->query("select franchise_id,franchise_name,login_mobile1 from pnh_m_franchise_info where franchise_id = ? ",$f_id)->row_array();
				$this->erpm->pnh_sendsms($fran_det['login_mobile1'],"Dear ".$fran_det['franchise_name'].",Invoice no-  ".implode(',',$invoice_nos)."is added in return list.Happy Franchising!!!",$f_id,$emp_id);
			}
		} 
	...
	function manifesto_pickup($from,$msg,$fran_ex)
	{
		...
		$pickup_empdet_res = $this->db->query('select pickup_empid,job_title2,c.territory_id,c.town_id  
														...
														join m_town_territory_link c on c.employee_id = b.employee_id and c.is_active = 1  
	 ...
	 */
		...
		//	$membr_id=@trim($member_id_arr[0]);
			...
			if(empty($membr_mobno))
				$this->pdie("Pleas enter Member Mobile number");
			if(empty($membr_name))
				$this->pdie("Member Name is Missing");
			$membr_id=(2003 .$this->p_genid(4)); 			//generate random member id   
			...
			if($this->db->query("select 1 from pnh_member_info where pnh_member_id=?",$membr_id)->num_rows()==0)
			{
				...
				if($this->db->affected_rows()>=1)
				{
					$this->erpm->pnh_sendsms($membr_mobno,"You have registered successfully with storeking,member id:$membr_id",$fran['franchise_id'],$membr_id,0);
					echo $membr_mobno,"You have registered successfully with storeking,member id:$membr_id";
					$this->pdie("Member Registered Successfully");
				}
			}
		...
		function is_mbr_regd($from,$msg)
		{
			...
			if(empty($membr_mobno)|| strlen($membr_mobno)<10)
				$this->pdie("Pleas enter a valid mobile number");
		...
		function fran_alloting_voucher_tomem($from,$msg,$fran)
		{
			...
			//if voucher book is alloted to franchise check is book activated to franchise
			if($is_voucher_assigned_tofran->num_rows())
				$is_voucher_alloted=$this->db->query("select * from pnh_t_voucher_details where is_alloted=1 and status=2 and franchise_id=?",$fran['franchise_id'])->result_array();

			if(empty($is_voucher_alloted))
				$this->pdie("The alloted Voucher Book is not activated yet!!! Pleas make a voucher value payment and continue franchising");
				//check member is registerd
			$is_membr_valid=$this->db->query("select * from pnh_member_info where mobile=?",$member_phno)->row_array();
			if(empty($is_membr_valid))
				$this->pdie("Member Not Registered.");
			...
			{
				$is_voucher_already_activated=$this->db->query("select * from pnh_t_voucher_details where (is_activated=1 or status=3) and voucher_serial_no=?",$v);
			}
			if($is_voucher_already_activated->num_rows()!=0)
				$this->pdie("voucher is already activated ");
			
			foreach($voucher_slnos as $v)
			{
				$is_alloted=$this->db->query("select * from pnh_t_voucher_details where franchise_id=? and voucher_serial_no=? and is_alloted=1 and status=2",array($fran['franchise_id'],$v))->row_array();
				if($is_alloted)
				{
					$voucher_value+=$is_alloted['value'];
			...
			if($redeemd_voucher_count==$voucher_count)
			{
				...
				{	$is_alloted=$this->db->query("select * from pnh_t_voucher_details where franchise_id=? and voucher_serial_no=? and is_alloted=1 and status=2",array($fran['franchise_id'],$v))->row_array();
					...
					$this->db->query("update pnh_t_voucher_details set is_activated=1,member_id=?,activated_on=now(),status=3 where voucher_serial_no = ? and franchise_id=?",array($is_membr_valid['pnh_member_id'],$v,$fran['franchise_id']));
					//if($this->db->affected_rows()>0)
					$this->db->query("insert into pnh_voucher_activity_log(voucher_slno,franchise_id,member_id,transid,debit,credit,status)values(?,?,?,?,?,?,0)",array($v,$fran['franchise_id'],$is_membr_valid['pnh_member_id'],0,0,$is_alloted['customer_value']));
				}

				$membr_name=$is_membr_valid['first_name'].' '.$is_membr_valid['last_name'];
				$this->erpm->pnh_sendsms($member_phno,"Dear $membr_name,Voucher-$voucher_slno of value $voucher_value Rs is successfully credited to you account.Happy Shopping",$member_phno,0,0);
				echo "Dear $membr_name,Voucher-$voucher_slno of value $voucher_value Rs is successfully credited to you account.Happy Shopping.";
				$this->pdie("Voucher is alloted to member successfully.Happy Franchising");
			}
				
			else
				$this->pdie("Invalid Voucher slno entered");
		}
		/**
		...
		function create_prepaidorder($from,$msg,$membr)
		{
			...
			if(empty($voucher_code))
				$this->pdie("Pleas Enter Voucher Scratch Code");
			...
			if($voucher_count!=$valid_voucher_count)
				$this->pdie("Pleas Enter Correct Voucher Code");
			...
			if($voucher_value<$c_total)
			{
				...
				{	
					...
					$this->pdie("Insufficient balance! Total value of purchase is Rs $c_total and voucher value is Rs $voucher_value $vbal ");
				}
					


				$part_redeemed_vouch_res=$this->db->query("select customer_value,voucher_code from pnh_t_voucher_details where status=5 and customer_value!=0 and member_id=?",$is_strkng_membr['pnh_member_id']);
			...
			$this->pdie("your balance after purchase is Rs $cus_value.Happy Shopping");
		...
		function credit_on_imei_newmem($from,$msg,$fran)
		{
			...
			if($total_margin_cr)
			{
				...
				if($this->db->query("select 1 from pnh_member_info where pnh_member_id=?",$imei_memid)->num_rows()==0)
				{
					...
					$this->db->insert('pnh_member_info',$inp_data);
					if($this->db->affected_rows()>=1)
						$this->erpm->pnh_sendsms($imei_mobno,"Member Registered Successfully.",$imei_memid);
					foreach($is_valid_imeino_res->result_array() as $valid_imei)
					{
						$this->db->query("update king_orders set member_scheme_processed=1 where id=? and member_scheme_processed=0",$valid_imei['orderid']);
						$this->db->query("insert into t_invoice_credit_notes(franchise_id,type,ref_id,invoice_no,amount,created_on)value(?,2,?,?,?,now())",array($valid_imei['franchise_id'],$valid_imei['imei_ref_id'],$valid_imei['invoice_no'],$valid_imei['msch_amt']));
